...
export default function Hero({ setIsModalOpen, isModalOpen }) {
  const { student, addStudent } = useStore()
   const [presentStudent,setPresentStuden]=useState([])
  const [user, setUser] = useState(initialFormState);
  const [studentId,setStudentId]=useState("")
  const [hendlePresent,setHendlePresent]=useState([])
  const getData = async () => {
    try {
  ...
  const onChange = (id) => {
    if (hendlePresent.includes(id)) {
      const data = hendlePresent.filter((item) => item !== id);
      setHendlePresent(data);
    } else {
      setHendlePresent([...hendlePresent, id]);
    }
  };
const handlePresent=async(id)=>{
  ...
  if (docSnap.exists()) {
    ...
    await setDoc(doc(db, "attendence", VenueId), presentStudent);
      ...
      <Table dataSource={student} className="myTable">
        ...
        />
         <Column
            ...
            render={(venueId) => (
              <div>
              <Button className='bg-green-500 text-white' onClick={()=>handlePresent(venueId)}>Present</Button>
              <Button className='bg-red-500 text-white' onClick={()=>handleAbsent(venueId)}>Absenst</Button>
              </div>
            )}
          />
      </Table>
